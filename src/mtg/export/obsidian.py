...
from mtg.analysis.ownership import BUY, MARK, OWN, PRECON, Row, summary
...
def deck_data(deck: Deck, rows: list[Row], dp: DeckPrice, unresolved: list[str], today: str) -> str:
    ...
    out = [
        ...
        "",
        f"**{deck.count()} cards** · {MARK[OWN]} {s[OWN]} owned · {MARK[PRECON]} {s[PRECON]} in a precon box · "
        f"{MARK[BUY]} {s[BUY]} to buy",
        "",
        "| | Paper | MTGO |",
        "|---|---|---|",
...
def collection_summary(holdings: list[Holding], catalog: Catalog, today: str) -> str:
    ...
    precons = Counter(h.source for h in holdings if h.source.startswith("precon:"))
    lines = [
